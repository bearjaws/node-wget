...
function download(src, output, options, _parentEvent, redirects) {
    ...
    if(!srcUrl.protocol || srcUrl.prototype === null) {
        downloader.emit('error', 'Cannot parse url protocol for ' + src + ', please specify either HTTP or HTTPS');
        return false;
    }
    ...
    req = request({
        ...
        if(res.statusCode === 301 || res.statusCode === 302 || res.statusCode === 307) {
            ...
            if(res.headers.location.indexOf('/')===0) {
                newLocation = srcUrl.protocol+'://'+srcUrl.hostname + (srcUrl.port ? ':' + srcUrl.port:'') + res.headers.location;
            }
        ...
        if (res.statusCode === 200) {
            ...
            res.on('data', function(chunk) {
                ...
                downloader.emit('progress', calculateProgress(fileSize, downloadedSize));
